...
- `text`: Labels and annotations (bound or standalone)
...
- **Text binding**: Use `boundElements` + `containerId` to bind text to containers
...
Both shapes (rectangle, ellipse, diamond) and arrows use a separate text element bound via `containerId` / `boundElements`:
...
**Key rules for bound text positioning:**
- `text.x` = `shape.x + (shape.width - <estimated text width>) / 2`
- `text.y` = `shape.y + (shape.height - 25) / 2`
- Text width ≈ `text.length * fontSize * 0.6`
- Text height is always 25 for single-line labels
- Use `fontFamily: 5`, `textAlign: "center"`, `verticalAlign: "middle"`

**Never** use standalone (unbound) text elements for labels on arrows or inside shapes. **Never** rely on the `text` property alone on a shape element — it will not display.

### Arrow to Shape Binding

When an arrow connects two shapes (e.g., flow between boxes), **bind the arrow to both shapes** so it moves with them when dragged. This requires a **bidirectional reference**:

1. Arrow carries `startBinding` and `endBinding` pointing to shape IDs
2. Each shape lists the arrow ID in its `boundElements` array as `{"id": "<arrow_id>", "type": "arrow"}`
...
| Text detaches from arrows when moving | Use bound text with `containerId` + `boundElements` instead of standalone text elements |
| Arrows don't follow shapes when dragged | Ensure `startBinding`/`endBinding` are set on the arrow AND the arrow ID is in each shape's `boundElements` |
...
- [ ] **Arrow and shape labels use `containerId` + `boundElements` (never standalone text)**
- [ ] **Arrows connecting shapes use `startBinding`/`endBinding` + shape `boundElements` sync**
...
3. **Add connecting arrows**:
   ...
   python skills/excalidraw-diagram-generator/scripts/add-arrow.py \
     <diagram-path> <from-x> <from-y> <to-x> <to-y> [--label "Text"] [--from-id ID] [--to-id ID] [--style solid|dashed|dotted] [--color HEX]
   ```
   - Edit via `.excalidraw.edit` is enabled by default to avoid overwrite issues; pass `--no-use-edit-suffix` to disable.
   - Use `--from-id` and `--to-id` to bind the arrow to existing shape elements (arrow follows shapes on drag).
   
   **Examples**:
   ```bash
   # Simple arrow from (300, 250) to (500, 300) — not bound to shapes
   python scripts/add-arrow.py diagram.excalidraw 300 250 500 300
   
   # Arrow bound to shapes — moves with them when dragged
   python scripts/add-arrow.py diagram.excalidraw 300 250 500 300 --from-id box-a --to-id box-b
   
   # Arrow with label + bindings
   python scripts/add-arrow.py diagram.excalidraw 300 250 500 300 --from-id box-a --to-id box-b --label "HTTPS"
   
   # Dashed arrow with custom color + label
...
4. **Workflow summary**:
   ...
   # Step 3: Add connecting arrows (bound to shapes)
   python scripts/add-arrow.py my-diagram.excalidraw 250 200 300 250 --from-id Internet-gateway-text --to-id VPC
   python scripts/add-arrow.py my-diagram.excalidraw 300 300 400 300 --from-id VPC --to-id ELB
   python scripts/add-arrow.py my-diagram.excalidraw 400 330 500 350 --from-id ELB --to-id EC2
   python scripts/add-arrow.py my-diagram.excalidraw 500 380 600 400 --from-id EC2 --to-id RDS
   ```
...
# Step 4: Add connecting arrows (bound to shapes)
python scripts/add-arrow.py my-aws-diagram.excalidraw 200 150 250 200 --from-id <igw-id> --to-id <vpc-id>
python scripts/add-arrow.py my-aws-diagram.excalidraw 265 230 350 250 --from-id <vpc-id> --to-id <elb-id>
python scripts/add-arrow.py my-aws-diagram.excalidraw 415 280 500 300 --from-id <elb-id> --to-id <ec2-id>
python scripts/add-arrow.py my-aws-diagram.excalidraw 565 330 650 350 --from-id <ec2-id> --to-id <rds-id> --label "SQL" --style dashed
